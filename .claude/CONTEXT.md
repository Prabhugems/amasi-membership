...
|---|---|---|---|
| 2026-08-07 | `9dd3cf7` | `bulk-draft-reminders.ts`'s exclusion set anti-joined idle drafts only against `membership_applications`, never `members`. Approved members with a stray `in_progress` draft (created by `/api/otp/send` on any `/apply` revisit with a `membershipType` — that route never checks `members`) had no matching `membership_applications` row, so nothing excluded them from "incomplete application" reminder emails despite already being members. | Added a `members` table anti-join to `bulk-draft-reminders.ts`, mirroring the pattern `cleanup-drafts/route.ts` already used. **Pattern: two sibling draft-cleanup jobs drifted — a members-table exclusion lesson learned in one was never ported to the other.** |
