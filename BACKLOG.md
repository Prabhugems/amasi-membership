...
> **Update 2026-06-24:** the lint/type-debt rows below were each blocked on "the pre-commit hook surfaces pre-existing errors → `--no-verify`." That root cause is fixed — lint-staged is now diff-scoped (see "Tooling: lint-staged scope" below). Commits to these files no longer trip pre-existing errors, so none of these rows are urgent anymore. The remaining `@typescript-eslint/no-explicit-any` cleanup in each file is now opt-in quality work, best paired with the relevant refactor noted in each row.
...
- [x] **DONE 2026-06-24.** Scoped lint-staged to lint only changed lines, not whole files. Pre-commit (lint-staged) now runs `eslint --config eslint.config.staged.mjs --fix`, where the staged config layers `eslint-plugin-diff`'s `flat/staged` processor on top of the base `eslint.config.mjs`. That processor filters lint messages down to lines present in `git diff --staged`, so the hook only blocks on problems introduced by the current commit; pre-existing debt on untouched lines no longer triggers `--no-verify`. Editor and `npm run lint` are unchanged (still use `eslint.config.mjs`, full-file) so all errors remain visible. **Note:** the package named in earlier rows (`lint-staged-eslint-diff`) does not exist on npm — `eslint-plugin-diff` is the real, maintained tool. Verified: a comment-only change to a file with 11 pre-existing errors (`src/lib/ai-decision-log.ts`) fails the full config (exit 1) but passes the staged config (exit 0); a newly-added `any` is still flagged. Originally triggered by `144b169` (middleware allowlist coverage test) where `--no-verify` landed 23 comment-only lines that tripped 37 pre-existing `@typescript-eslint/no-explicit-any` reports.
