/**
 * One-shot recovery — JAHNAVI MAHANTA (jahnavimhnt@gmail.com, LM).
 *
 * Context (2026-06-21):
 *   Orphan-payment variant of the paid-no-submission pattern.
 *     - 2026-06-20 11:14 UTC the applicant paid ₹4,230 — Razorpay
 *       pay_T3rhQl5ksnOYFm / order_T3rhDXmy1Ktiak. `membership_payments` has the
 *       row with status='paid', application_id=NULL.
 *     - The draft that minted that order was hard-deleted (no row references the
 *       order), so the standard recover script can't read payment_id from
 *       step_data the way the Bonde case does.
 *     - On 2026-06-21 the applicant started OVER: a fresh LM draft
 *       (b0f18fee-2a05-4750-ba9b-4785b3cbdcc1) with a COMPLETE LM doc set
 *       (profile, MCI cert, PG degree cert, ASI member cert — all extracted) and
 *       email verified (otp_codes: 3 verified, last 2026-06-21 15:30 UTC). She
 *       reached the payment step (step_data.pre_payment_checkpoint) and was
 *       correctly BLOCKED by the create-order orphan guard (409 PAID_UNLINKED) —
 *       so she was NOT double-charged, but also could not finish on her own.
 *
 * This script therefore differs from recover-bonde in ONE way: the payment id is
 * pinned here (PAYMENT_ID) instead of read from draft step_data, because the
 * current draft legitimately has no payment_id. Everything else mirrors
 * /api/applications/submit minus auto-approval: reconstruct from the current
 * draft's formData+uploads, re-verify the orphan payment is paid+unlinked, score,
 * insert as pending_review, link the payment, soft-complete the draft.
 *
 * Usage:
 *   npx tsx scripts/recover-jahnavi-orphan-payment-2026-06-21.ts            # dry run
 *   npx tsx scripts/recover-jahnavi-orphan-payment-2026-06-21.ts --commit   # do it
 *
...
const EMAIL = "jahnavimhnt@gmail.com"
// Pinned: the June-20 orphan payment. NOT sourced from step_data (the current
// draft never carried it — the original draft was hard-deleted).
const PAYMENT_ID = "pay_T3rhQl5ksnOYFm"
const EXPECTED_ORDER_ID = "order_T3rhDXmy1Ktiak"
const ADMIN_ACTOR = "payment-recovery-orphan-2026-06-21"

const RECOVERY_NOTE =
  "Payment-recovery (2026-06-21): applicant paid ₹4230 on 2026-06-20 (membership_payments.status='paid', " +
  "pay_T3rhQl5ksnOYFm) but the draft that minted the order was deleted; applicant re-did the whole " +
  "application (fresh LM draft, complete docs, email verified) and was blocked from re-paying by the " +
  "create-order orphan guard. Promoted manually from the current draft's step_data and linked to the " +
  "existing June-20 payment (no second charge). Routed to manual review; verify docs before approving."
...
async function main() {
  ...
  // --- 2b. Confirm email was verified server-side (step_data flag is unsynced) ---
  ...
  // WS-C finalize-or-insert (mirrors recover-bonde): if an early pending_payment
  // skeleton exists for this email, finalize it in place; else INSERT.
  ...
  const row = buildApplicationRow({
    ...
    emailVerified: true, // confirmed via otp_codes above
  ...
  else console.log("✓ June-20 payment linked to application")
  ...
  await logAdminAction({
    ...
    details: {
      ...
      amount: pay.amount,
      draftId: draft.id,
      reason: "orphan_payment_applicant_redid_application",
    },
  })
