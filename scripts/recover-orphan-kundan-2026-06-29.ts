/**
 * One-shot recovery — KUNDAN RAI (kundan.rai653@gmail.com, LM).
 *
 * Context (2026-06-29):
 *   Orphan-payment variant of the paid-no-submission pattern.
 *     - 2026-06-29 02:35 UTC the applicant paid ₹4,230 — Razorpay
 *       pay_T7Hfs3lDkakLUC / order_T7HfkZKEalQGee. `membership_payments` has the
 *       row with status='paid', application_id=NULL (orphan).
 *     - The applicant's draft (723ca620-89e0-41e5-b0b5-d960443c45e4) is still
 ...
 *       verified. step_data carries payment_verified=true, paid_pending_submission=true,
 *       submit_failed=true (submit_failed_at 2026-06-29T02:36:21Z) — the final
 *       submit failed seconds after the verified payment, so no application row
 *       was ever created.
 *
 *   VERIFY-BEFORE-APPROVE: the ASI member certificate reads "Dr. KUNDAI RAI" and
 *   the draft formData.firstName is "KUNDAI", while the MCI/PG certs say
 *   "Kundan Rai". Almost certainly a typo of "Kundan". The reconstructed row will
 *   carry name "KUNDAI RAI" — correct the first name in /pending before approving.
 *
 * Mirrors /api/applications/submit minus auto-approval (the exact behaviour of
 * POST /api/admin/orphan-payments/promote, run as a one-shot here): reconstruct
 * from the current draft's formData+uploads, re-verify the orphan payment is paid
 * + unlinked, score, insert as pending_review, link the payment, soft-complete
 * the draft.
 *
 * Usage:
 *   npx tsx scripts/recover-orphan-kundan-2026-06-29.ts            # dry run
 *   npx tsx scripts/recover-orphan-kundan-2026-06-29.ts --commit   # do it
 *
...
const EMAIL = "kundan.rai653@gmail.com"
const PAYMENT_ID = "pay_T7Hfs3lDkakLUC"
const EXPECTED_ORDER_ID = "order_T7HfkZKEalQGee"
const ADMIN_ACTOR = "payment-recovery-orphan-2026-06-29"

const RECOVERY_NOTE =
  "Payment-recovery (2026-06-29): applicant paid ₹4230 on 2026-06-29 (membership_payments.status='paid', " +
  "pay_T7Hfs3lDkakLUC) with payment_verified=true, but the final submit failed (draft submit_failed=true, " +
  "paid_pending_submission=true) so no application was created. Promoted manually from the current draft's " +
  "complete LM step_data and linked to the existing payment (no second charge). VERIFY NAME before approving: " +
  "ASI cert + form read 'KUNDAI RAI' vs 'Kundan Rai' on MCI/PG certs — likely a typo of 'Kundan'. " +
  "Routed to manual review; verify docs/name before approving."
...
async function main() {
  ...
  await logAdminAction({
    ...
    details: {
      ...
      amount: pay.amount,
      draftId: draft.id,
      reason: "orphan_payment_submit_failed",
    },
  })
