...
function placeOf(r: MmasRow): string | null {
  return r.course_place ?? r.fallback_place ?? null
}

function rowMatchesQuery(r: MmasRow, needle: string): boolean {
...
function buildCsv(rows: MmasRow[]): string {
  const head = [
    "AMASI Number",
    "Name",
    "Email",
...
function CourseBadge({ row }: { row: MmasRow }) {
...
function DetailSheet({
  ...
  const emailCert = useMutation({
    mutationFn: async () => {
      const res = await fetch("/api/admin/mmas/email-cert", {
        method: "POST",
  ...
  const saveDispatch = useMutation({
    mutationFn: async () => {
      const res = await fetch("/api/admin/mmas/dispatch", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["admin-mmas-list"] })
    },
    onError: (e: Error) => toast.error(e.message),
  })
  ...
  const generateVerifyLink = useMutation({
    mutationFn: async () => {
      const res = await fetch("/api/admin/mmas/verify-link", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          amasi_number: row.amasi_number,
  ...
  const others = (otherCreds.data?.credentials ?? []).filter(
    (c) => !(c.credential_type === "MMAS" && c.year === row.year)
  )
  ...
  return (
    <>
      ...
      <aside
        ...
        role="dialog"
        aria-label="MMAS holder details"
      >
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="relative overflow-hidden border-2 border-amber-200/80 dark:border-amber-400/30 rounded-2xl bg-gradient-to-br from-amber-50 via-white to-amber-50/40 dark:from-amber-500/15 dark:via-slate-950 dark:to-amber-500/5 shadow-sm">
            ...
            <div className="relative p-4">
              ...
              <div className="space-y-1">
                {row.course_name ? (
                  ...
                  <div className="text-muted-foreground italic">No category linked</div>
                )}
              </div>
            ...
            <div className="px-4 pb-4 flex gap-2">
              ...
              >
                <Link href={`/member/mmas-certificate?id=${row.amasi_number}`} target="_blank">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-1.5 text-sm">
              <Link
                href={`/member/mmas-certificate?id=${row.amasi_number}`}
                target="_blank"
                className="flex items-center justify-between px-3 py-2 rounded-lg border hover:bg-muted/50 transition-colors"
              >
                <span>MMAS certificate</span>
                <ExternalLink className="h-3.5 w-3.5 text-muted-foreground" />
              </Link>
              <Link
                href={`/api/credential?type=MMAS&id=${row.amasi_number}`}
                target="_blank"
                className="flex items-center justify-between px-3 py-2 rounded-lg border hover:bg-muted/50 transition-colors"
              >
...
export default function AdminMmasPage() {
  ...
  const [selected, setSelected] = useState<MmasRow | null>(null)
  ...
  const eligibleQuery = useQuery<{ eligible_count: number }>({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/admin/mmas/email-cert-bulk?year=${encodeURIComponent(year)}`)
  ...
  const { data, isLoading, isError } = useQuery<MmasApiResponse>({
    queryKey: ["admin-mmas-list"],
    queryFn: async () => {
      const res = await fetch("/api/admin/mmas")
      if (!res.ok) throw new Error("Failed to load MMAS data")
      return res.json()
    },
  })
  ...
  const handleExport = useCallback(() => {
    ...
    downloadCsv(`mmas-holders-${stamp}.csv`, csv)
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-start gap-4 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-muted-foreground mt-1.5 text-sm">
                Mastery in Minimal Access Surgery
      ...
      <div className="border rounded-2xl bg-card shadow-sm overflow-hidden">
        <div className="p-4 border-b bg-gradient-to-b from-muted/30 to-transparent">
          <div className="relative group">
            ...
            <Input
              ref={searchRef}
              placeholder="Search name, AMASI #, mobile, email, category, place…"
              value={q}
              onChange={(e) => {
                setQ(e.target.value)
                setPage(1)
      ...
      {isLoading ? (
        <div className="border rounded-xl p-16 text-center bg-card">
          ...
          <p className="text-sm text-muted-foreground mt-3">Loading MMAS holders…</p>
        </div>
      ) : isError ? (
        ...
        <div className="border rounded-xl p-16 text-center bg-card">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {hasActiveFilters ? "Try adjusting search or filters." : "No MMAS holders yet."}
          </p>
          {hasActiveFilters && (
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5 text-right">
                      <div className="flex justify-end gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                        ...
                        >
                          <Link
                            href={`/member/mmas-certificate?id=${r.amasi_number}`}
                            target="_blank"
                          >
