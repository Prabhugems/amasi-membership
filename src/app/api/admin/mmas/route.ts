...
interface MmasResponseRow {
  ...
  // per-year convocation place fallback, from credential_templates
...
interface MmasResponse {
  rows: MmasResponseRow[]
  stats: {
    total: number
...
export async function GET() {
  ...
  try {
    ...
    if (skillCourses.length === 0) {
      warnings.push("skill_courses has no MMAS rows yet — categories won't display until seeded.")
    }
    ...
    const rows: MmasResponseRow[] = creds.map((c) => {
