...
// Template artwork is 1406x2096 (aspect ~0.671) — different from the FMAS
...
function MmasCertificateContent() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["mmas-certificate", id],
    queryFn: async () => {
      if (!id) return null
      const res = await fetch(`/api/credential?type=MMAS&id=${id}`)
  ...
  const handleDownloadPNG = async () => {
    ...
    try {
      ...
      link.download = `AMASI-MMAS-${cert.amasiNumber}-${cert.year}.png`
      link.href = canvas.toDataURL("image/png")
      link.click()
      toast.success("MMAS certificate downloaded as PNG")
    } catch {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      pdf.save(`AMASI-MMAS-${cert.amasiNumber}-${cert.year}.pdf`)
      toast.success("MMAS certificate downloaded as PDF (A4)")
    } catch {
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      if (!win) return
      win.document.write(`<html><head><title>MMAS Certificate - ${cert.amasiNumber}</title>
        <style>
  ...
  if (isLoading) return (
    <div className="text-center py-16">
      ...
      <p className="text-muted-foreground mt-3">Loading your MMAS certificate...</p>
    </div>
  )
  ...
  if (!cert) return (
    <div className="text-center py-16 space-y-3">
      <Award className="h-12 w-12 mx-auto text-muted-foreground/30" />
      <p className="text-muted-foreground">No MMAS credential on record for this AMASI number.</p>
  ...
  return (
    <div className="space-y-8 max-w-3xl mx-auto">
      ...
      <div className="text-center space-y-3">
        ...
        <div>
          <h1 className="text-2xl font-bold">MMAS Certificate</h1>
          <p className="text-muted-foreground text-sm">Mastery in Minimal Access Surgery &mdash; Dr. {cert.name}</p>
        </div>
      </div>
          ...
          <div className="overflow-auto flex justify-center">
            ...
            >
              <img
                ...
                alt="MMAS certificate"
                crossOrigin="anonymous"
                style={{ width: "100%", height: "100%", position: "absolute", top: 0, left: 0 }}
              />
              <div style={{
                position: "absolute",
                top: `${typeof cert.nameTopPct === "number" ? cert.nameTopPct : 42}%`,
                left: 0,
                right: 0,
                textAlign: "center",
...
export default function MmasCertificatePage() {
  return (
    <>
      <AdminBackLink />
      <Suspense fallback={
