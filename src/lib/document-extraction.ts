/**
 * Shared document extraction library.
 * Contains all OCR logic: preprocessing, Claude Vision, fallback OCR, sanity checks,
 * expiry validation, and eligibility checks.
 *
 * Used by both /api/ocr/route.ts and /api/applications/resubmit/route.ts.
 */

import Anthropic from "@anthropic-ai/sdk"
import sharp from "sharp"
import { requiresExtraction } from "./document-keys"
...
export interface ExtractionInput {
  buffer: Buffer
  fileName: string
  docType: string   // should already be canonical
}

export interface ExtractionOutput {
  extracted: Record<string, any>
  eligibility: { eligible: boolean; reason: string; softBlock?: boolean } | null
...
export function detectSuspiciousExtraction(docType: string, extracted: Record<string, any>): string | null {
...
export function checkEligibility(docType: string, extracted: Record<string, any>) {
...
// ---------------------------------------------------------------------------
// Date parser (for expiry validation)
// ---------------------------------------------------------------------------
...
function validateExpiry(docType: string, extracted: Record<string, any>): string[] {
...
/**
 * Extract data from a document buffer using Claude Vision (primary) or OCR.space (fallback).
...
export async function extractDocument(input: ExtractionInput): Promise<ExtractionOutput> {
  ...
  // Early return for non-extraction doc types (photos, etc.)
  if (!requiresExtraction(input.docType)) {
    return {
      extracted: {},
      eligibility: null,
      expiryWarnings: [],
  ...
  try {
    const { buffer, fileName, docType } = input
    ...
    // Preprocess images (skip for PDFs)
    let sendBuffer: Buffer = buffer
    let sendMimeType = isPDF ? "application/pdf" : isPNG ? "image/png" : "image/jpeg"
    if (!isPDF) {
          ...
          messages: [{
            ...
            content: [
              isPDF
                ...
                : { type: "image" as const, source: { type: "base64" as const, media_type: sendMimeType as any, data: base64 } },
    ...
    // Expiry validation
    const expiryWarnings = validateExpiry(docType, extracted)
